...
* **24HZ** H10+newer: enable film standard 24.0 frame, rather than the default broadcast standard 23.976. The existing 24p mode(s) will have the new frame rate when this is enabled, all other video modes are unaffected. Input Data: 1-enable, 0-disable. e.g. `*24HZ=1`
* **64BT** H8-10/Max1: 12GB Chapter sizes on HERO8/9/10 and MAX cameras (default on H11+), 64GB chapters on MAX2. Note: Will not be active for QuickCaptures. Input Data: 1-enable, 0-disable WARNING: Larger chapters will not playback on camera or support USB transfers, but are compatible with desktop tools. Permanent required. e.g. `*64BT=1`
* **64BT** H11-13/Max2/M1: These camera default to 12GB Chapter sizes, but support larger. Note: Will not be active for QuickCaptures. Input Data: x-size in MBs, 0-disable. Permanent required. e.g. `*64BT=32000` for 32GB chapters.
* **ALLI** Max2/M1: Like 24HZ, this also makes 29.97 -> 30.0 and 59.94 -> 60.0. All integer frame frames. e.g. *ALLI=1
* **ARCH** H8-13/Max1-2: Archive mode: an ultra simplified video camera mode for novices documenting critical events, where you don’t want the camera mode modified. Either button will start and stop video capture. Input Data: 1-enable, 0-disable WARNING: only removable via the disable command. Permanent required. e.g. `*ARCH=1`
...
* **B001** thru **B999** Max2/M1: preset friendly B001 to B999 for bitrate control e.g. B180 in a preset name will be 180mbs
* **BASE** H8-13/Max1-2/M1: Base file name change, adding to beginning of the filename. Input: H10-13 supports wildcards within [ ]: like [yyyy-mm-dd] or [HH-MM-SS]  e.g. A-[yyyymmddHHMMSS] WARNING: GoPro App and cloud will not support renamed files. e.g. `*BASE="CamA-[yy-mm-dd]"`
* **BERS** H10-13/M1: Bypass ERS compensation, extremely rare usecases. Input Data: 0-display, 1-enable, 2-enable only with EIS off e.g. `*BERS=2`
...
* **BITR** H10+newer: set the compression in Mb/s for the Protune High Bitrate setting. Normally this would be around 100Mb/s, however higher (or lower) rates may be achieved with newer SD Cards.Input Data: MB/s from 2 to 200. e.g. `*BITR=180`
...
* **BOOT** H7+newer: Boot command can be dangerous, but a safe way to use them is to store the script on the SD Card. So the boot command would be `*BOOT="!Lbt"` which attempts to load a file bt from the MISC folder of the SD card. So creating boot command should always use the formatting `*BOOT="!Lbt"!SAVEbt="your script"` e.g. `*BOOT="!Lbt"!SAVEbt=!12:00NmP!S!1R`
* **BURN** H8-13/Max1/M1: Add date, time and other burn-in text to your video. Input Format (0,height)[yyyy-mm-dd HH:MM:SSaa] examples:   (0,30)Local Time: [HH:MMaa]   (0,26)Local Time: [HH:MM:SSAA]   (0,40)©[yyyy]\ XYZ Inc.   (0,24)Shoot Date [yy/mm/dd]
* **BRNP** H8-13/Max1/M1: The burn-in position for overlays or LOGO display. Input Data (select 1 of 8): TL, TC, TR, ML,BL, BC, and BR\nT-Top B-Bottom C-Center L-Left and R-Right.
* **BRNT** H8-13/Max1/M1: The overlays or LOGO display time in seconds. e.g. BRNT=0.016 will display the logo or text overlays only on the first frame. Input Data: 0 is unlimited, or 0.01 to 3600 seconds.
* **BRNX** H8-13/Max1/M1: Offset the overlays or LOGO display with X,Y pixel coordinates. Input Data: x,y pixel coordinates.e.g. 120,40
* **BYPS** H10+newer: Bypass common pop-ups, such as resetting the time and date. Remember to set time and date if you remove the battery. Input Data: 1 to bypass, 0 for normal notifications. e.g. `*BYPS=1`
* **CBAR** H9-13/M1: enable a small 75% saturated color bars for video tools evaluation. Most cameras limit overlays to 4K30, 2.7Kp60 or lower. Input Data: 1 to enable, 0 to disable
* **COOL** H12-13/Max2/M1: offset white balance for the blue channel. $COOL=40 will add 40% more gain to the blue channel.
* **DAMP** H10-13/Max2: Control over the auto-exposure damping. Setting the DAMP to 1 is the default, setting to 10 would slow the camera’s exposure adjustments 10X.Input Data: 0.1 to 10000. e.g. `$DAMP=10`
* **DAUD** H10-13/M1: Disable Audio in video captures, all MP4 files will have no audio track. Application: high bit-rate drones video.Input Data: 1 to disable audio, 0 to re-enable
* **DBGL** H8+newer: Enables more debug logging. Labs saves it logs to the SD card under MISC/qrlog.txt. Using this extension increases the details and logged events. e.g. To see more logging from boot up use `*DBGL=1`
* **DIST** H13/Max2: Time-Lapse Video Distance, x is a GPS travel distance between exposures in meters, rather than time for an in-camera computed timelapse. So `$DIST=5` will set a 5 meter interval for TLV. One exception is when x=1, 10 meters will be used, 1 meter is too noisy (GPS errors.) So this feature is good for 2 meters of larger. For shorter distances or a higher speed, you the new TLV sample rate for 4Hz (p4), for the best distance precision. e.g. `mTr5Xp4$DIST=5`
...
* **DLRV** H10-11/M1: Disable LRV file creation. Application: high bit-rate drones video. Input Data: 1 to disable LRVs, 0 to re-enable. e.g. `*DLRV=1` to permanently disable LRV files.
* **DPTS** Max2 only: stitch depth change smoother, 1 - fast, 15 - smooth. 360 experimental.
* **DPTP** Max2 only: stitch depth change speed, 1 - slow, 7 - fast (like DPTS with a different implementation). 360 experimental.
* **DPTL** Max2 only: stitch depth lock stitch after x seconds, 1 - immediate, 2-99 - seconds, 100-99999 - milliseconds. 360 experimental.
* **DSPC** H7+newer: Labs text display contrast. Set the background opacity for Labs text on the LCD. Input Data: Number from 0-transparent to 6-completely opaque.
* **DSPL** H7+newer: Labs text display length. How long to display each Labs message, like owner information (OWNR). The default is 1 second. Input Data: Number from 1-9 seconds or 10-9999 milliseconds.
...
* **EVBS** H10+newer: This is an EV compensation value that works with webcam and livestreaming, it can be changed live (with QRDR=1) and it is global, adding the any existing EV control in your presets.Input Data: range -4 to 4.
* **EXPQ** H11-13/Max1-2: Min and Max Shutter speed, 1/x format. Input Data: 24 to 8000, representing 1/24 and 1/8000s, 0 to disable. e.g. `$EXPQ=120`
* **EXPS** H11+newer: Video exposure values: ISO and Shutter speed, rendered to the LCD. Handy for those using ND Filters. Input Data: 1 to enable, 0 to disable (2 smoother rendering.) 
* **EXPX** H10-13/Max1-2: maX EXPosure time (longest shutter), 1/x format. Input Data: 24 to 8000, representing 1/24 and 1/8000s, 0 to disable. e.g. `$EXPX=120`
* **EXPN** H10-13/Max1-2: miN EXPosure time (shortest shutter), 1/x format. Input Data: 24 to 8000, representing 1/24 and 1/8000s, 0 to disable.  e.g. `$EXPN=240`
* **EXPT** H7-13/Max1-2: Video Exposure Control through Maximum Shutter Angle for video modes. Can improve stabilization in low light. Input Data: Number 0 through 5 stops.  0-360° (camera default), 1-180°, 2-90°, 3-45°, 4-22.5° etc.
* **FAST** H10+newer: Faster and fewer Labs notifications on boot. Input Data: 1-enable, 0-disable. e.g. `*FAST=1`
* **FEAT** H10+newer: Displays Labs enabled Features for x seconds. Input Data: x-seconds.  e.g. `$FEAT=4`
...
* **GAMA** Max2 only: Gamma control for easier HDRI work, gamma equivalent of LOGB. e.g. $GAMA=2.2 for standard 2.2 gamma curve.
* **GCMP** H11-13/Max1-2: Disable Gryo Compensated Exposure.  When GoPro camera modes, it normally uses a faster shutter for improved stabilization, this control disables this feature.Input Data: 1 to disable gyro comp, 0 - default
* **GPSL** H13/Max2/M1: enable continous GPS location logging, even when not capturing, so the capture can log more of you path and adventures, simply have the camera on. Path is saved by day into MISC/GPS-yy-mm-dd.gpmf file. GPSL=1 logs everything, GPSL=x will only log movement great than x meters from the last stored location. e.g. $GPSL=10 to log evert meters.
* **GUID** H11+newer: On screen guides lines for output aspect ratios. e.g. \$GUID=2.35, set shooting guides for cinescope. Input Data: List of aspect ratios (up to 8) as real numbers, so 16:9 is 1.778 and 9:16 is 0.5625.  e.g. `$GUID=1.778,0.5625`
* **GUIC** H11+newer: Color/brightness of guides lines. Input Data: List of guide brightness (up to 8).  e.g. `$GUIC=10,255`
...
* **HIST** H7+newer: Enable a histogram with with a range of contrast options. Input Data: Number 1 to 11 enables the histogram with contrast 1 through 11, 0 will disable it. e.g. `$HIST=5`
* **HSTP** H11+newer: Histogram position and size. A negative position will place the histogram on the front LCD. Input Data: x,y,size as percentages. e.g. `$HSTP=16,8,44`
* **HNDL** H7+newer: Set the camera ID to limit which cameras to respond to a QR Code. Used with the hZ command, where Z is the bit mask for which cameras will follow the command. Input Data: ID Number from 1-31. Usage: h6mP!S ← runs on cameras with IDs 2 or 3.
* **HSTO** H9-13/M1: Number minutes controlling the length of the Hindsight timeout, changing from the default for 15 minutes. Input Data: minutes from 5-1440. e.g. `*HSTO=180`
* **IFRM** Max2 only: Change the HEVC compression to use I-Frame only (experimental) e.g. *IFRM=1
...
* **JOIN** H7-13/Max2/M1: Setup storing your WiFi network credentials.Input Data: String in the format your_ssid:your_passphase e.g. HomeWiFi:Pass1234
...
* **LEVL** H11+newer: enable on-screen spirit level. Input Data: 1-9 sets the size of the level, 0 to disable.  e.g. `*LEVL=5`
* **LLTZ** H7+newer: Manual location: using Sunset/Sunrise triggers without waiting for a GPS lock. Input Data: numbers in the format latitude,longitude,timezone e.g. `$LLTZ=33.126,-117.327,-8.0`
* **LOGB** H11-13/Max2: Extremely advanced users. Change the Log base for very flat encoding. Color Flat is Log base 113. Math:out = log(in*(base-1)+1)/log(b). Should be combined with Flat and 10-bit settings. Input Data: base,[offset]e.g. 200,-128 or 400. e.g. $LOGB=400 for GPLog equivalent
* **LOGO** H9-13/M1: Activate a small logo (under 64K pixels) to render within the video. There must be a PNG file stored on the SD card in the MISC folder. Most cameras limit overlays to 4K30, 2.7Kp60 or lower. Input Data: String e.g. Logo.PNG for the name of your image
* **LOWL** Max2 only: Low Light more, working the same as GCMP, this will bias towards a longer shutter for lower noise, but will less stabilization. e.g. *LOWL=1
...
* **NR01** H11+newer: Noise reduction control, for advanced user intending noise reduction in post. Likely needs a higher BITR. Input Data: 1-100, is the default full denoise processing in camera. 1 - leaves the most noise. 0 - disables e.g. `$NR01=1` for maximum noise (denoise off)
* **NR02** thru **NR09** Max2/M1: preset friendly NR01 to NR99 for noise reduction percentage e.g. NR25 in a preset name will be 25% noise reduction.
* **OLTC** H12-13: Offset for LTC timecode in frames, enabling calibration you are seeing any offset..Input Data: x - frames.
* **OWNR** H7+newer: Display Owner information. This written into every MP4 and JPGs metadata, and MISC/GoPro-owner.txt file.Input Data: String up to 64 characters using '\\n' for a newline. Permanent required.
* **OWNr** H7+newer: Owner information, not displayed. This written into metadata, and MISC/GoPro-owner.txt file.Input Data: String up to 64 characters using '\\n' for a newline. Permanent required.
* **PMSG** H11+newer: Message to display during a Labs looping script, so you know the camera is running something custom. Input Data: Any label less than 20 characters, or empty to disable.
* **PRES** H12+newer: Create a new preset with custom name and icon. Input Data: Icon number 0 thru 32, preset name up to 15 characters. e.g. `$PRES="14,Scuba"` On HERO12 or later, present name that use FourCC commands with enable those command only when the user selects that preset.  e.g. `$PRES="14,DIVE WBLK"` creates a preset called "DIVE WBLK" that enables the DIVE and WBLK extensions. Note: While oMPRES and $PRES are using "temporary" metadata style commands, the preset created is not temporary, it will remain until the user removes it.
* **PRXY** H10-13/M1: Store LRV files as NLE ready proxies. Normally a camera will encode an LRV (Low Res Video) for every MP4, with this enabled LRVs are made MP4s within a subfolder. Input Data: 1-move LRVs, 2-move THMs, 3-both, (v2.1) 4-No _Proxy name. 0-disable
* **QRDR** H7+newer: Detect QR Codes while recording and during Quik previews. This also enables Labs BOOT commands to work with QuikCapture in recent models. Can be used for changing a video burn-in message in the middle of a live-stream. This also allows you to end a capture via a QR Code (command: !E).  Input Data: 1-enable, 0-disable
...
* **SPED** H8+newer: SD Card Speed Test. Data rates should have minimums above 120Mb/s is you want to reliably capture the high bitrate modes.Input Data: 1-9, number of times to run the test.
...
* **SYNC** H9+newer: GPS time/timecode sync. With a good GPS signal, the camera’s internal clock will be precisely updated to the millisecond. It will not update the hour or date, preserving your current timezone offset. Input Data: 1-enable, 0-disable. Permanent required. e.g. `*SYNC=1`
...
* **TONE** H10-13/Max2: Tone-mapping controls. Input Data:   0 - current defaults   1 - global tone-mapping only   2 - both tone-mapping   3 - disable all tone-mapping
* **TUSB** H10+newer: Trust USB power. Some USB power sources may report less than they are capable. This modification assumes the USB Power source is 2A minimum, and disables the testing. If you use TUSB with an inadequate power source, expect capture failures. e.g. `*TUSB=1`
* **WAKE** H9+newer: Conditional wake on any power addition. Inserting a battery or the connection of USB power, will boot up the camera to continue a script after a power failure.  Input Data: 1-wake if there is a delay action pending, 2-wake on power, 0-disable  e.g. `*WAKE=2`
* **WARM** H12+newer: offset white balance for the red channel. WARM is good to use with DIVE, so you can improve the red respond for deeper dives. `$WARM=50` will add 50% more gain to the red channel.  
...
* **WBLK** H11+newer: White balance Lock upon capture. Allows the convenience of auto white balance, without the risk of WB change during capture. Input Data: 1-enable WB Lock, 0-disable
* **WIDE** H11-13/Max2: A wide gamut color profile, this supports all in-camera white balancing. Like using white balance Native, without as much post color work. Input Data: 1-enable WIDE gamut, 0-disable e.g. `$WIDE=1`
* **ZONE** H9+newer: Set the time zone for use with SYNC. Input Data: time zone offset in minutes.
